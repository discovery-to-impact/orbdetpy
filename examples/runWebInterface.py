...
app.layout = html.Div([
    ...
    html.Div([
        # Row 1: Info, Row 2: dropdowns for input file selection
        html.Div([
            html.Div(['ABOUT'], style = {'margin-bottom' : '5px'}),
            html.Div([
                ...
                follow to correct format outlined in docs/filefmt.rst""")
            ], style = {'margin-bottom' : '20px'}),
            html.Div(['PROCESS SELECTION'], style = {'margin-bottom' : '10px'}),
            dcc.Dropdown(
        ...
        # Create text to tell user to submit and clear
        html.Div(id = 'my-div'),
        # Create buttons to run and rest processes
        html.Div([
        ...
        html.Div([
            html.Div(['DATA CONFIGURATION'], style = {'margin-bottom' : '10px'}),
...
# Update run info
@app.callback(
    Output(component_id='run-info', component_property = 'value'),
    [Input(component_id='input-dropdown-list', component_property='value'),
    Input(component_id='submit-button', component_property='n_clicks'),
     Input(component_id='runChecks', component_property='value')]
)
def update_output_div(mT, n_clicks, runChecks):
    out = 'You will run {}'.format(runChecks) + ' on {}'.format(mT) + ' data'
    if n_clicks == 1:
        out = out + '\n' + "Process start : %s" % time.strftime("%Y-%m-%d %H:%M:%S")
        out = out + '\n' + 'Please wait ...'
    return out
