...
public class Simulation
{
    ...
    public Simulation(String[] cfgjson)
    {
    	...
    	for(int i = 0; i < cfgjson.length; i++)
    	{
    		...
    		states.get(i).simcfg = Settings.loadJSON(cfgjson[i]);

			if (states.get(i).simcfg.Propagation.Step <= 0.0)
				states.get(i).simcfg.Propagation.Step = 60.0;
	...
	for(int i = 0; i < states.size(); i++)
	{
		...
		Settings simcfg = states.get(i).simcfg;
		...
		states.get(i).tm = tm;
		states.get(i).prop = prop;
		states.get(i).inclextra = inclextra;
		states.get(i).simulmeas = simulmeas;
		states.get(i).skipunobs = skipunobs;
		states.get(i).prend = prend;
	...
	while (true)
	{
		for(int i = 0; i < states.size(); i++)
		{
			...
			AbsoluteDate tm = states.get(i).tm;
			AbsoluteDate prend = states.get(i).prend;
			NumericalPropagator prop = states.get(i).prop;
			boolean inclextra = states.get(i).inclextra;
			boolean simulmeas = states.get(i).simulmeas;
			boolean skipunobs = states.get(i).skipunobs;
		    ...
		    json.ObjectName = simcfg.SpaceObject.Name;
		    ...
		    states.get(i).dt = prend.durationFrom(tm);
		    states.get(i).tm = new AbsoluteDate(tm, Math.min(states.get(i).dt, simcfg.Propagation.Step));
	    
		}
		
		if (states.get(states.size()-1).dt <= 0.0) // to break out check dt for last object?
		break;
	}
