 ...
 * @since 9.0
 */
public class AngularRaDecRates extends AbstractMeasurement<AngularRaDec> {
    ...
    AngularRaDec ratesConstructor;
    
    public AngularRaDecRates(final GroundStation station, final Frame referenceFrame, final AbsoluteDate date,
        ...
        ratesConstructor = new AngularRaDec(station, referenceFrame, date,
    ...
    protected EstimatedMeasurement<AngularRaDec> theoreticalEvaluation(final int iteration, final int evaluation,
        ...
        final FieldVector3D<DerivativeStructure> staSatInertialVel = transitStateDS.getVelocity().subtract(stationDownlink.getVelocity());
        ...
        final FieldVector3D<DerivativeStructure> staSatReferenceVel = inertialToReferenceDownlink.transformVector(staSatInertialVel);

        DerivativeStructure staSatRangeRate = staSatReferenceVel.dotProduct(staSatReference).divide(staSatReference.getNorm());
        // Compute right ascension and declination
        
        final DerivativeStructure rightAscension = (staSatReferenceVel.getX().multiply(staSatReference.getY()).subtract(staSatReferenceVel.getY().multiply(staSatReference.getX()))).divide
        												(staSatReference.getX().pow(2).multiply(-1).subtract(staSatReference.getY().pow(2)));        
        
        //final DerivativeStructure baseRightAscension = DerivativeStructure.atan2(staSatReference.getY().divide((staSatReference.getX().pow(2).multiply(staSatReference.getY().pow(2))).sqrt()),
        //																		 staSatReference.getX().divide((staSatReference.getX().pow(2).multiply(staSatReference.getY().pow(2))).sqrt()));

        
        final DerivativeStructure declination        = ((staSatReferenceVel.getZ()).subtract((staSatRangeRate).multiply(staSatReference.getDelta().sin())))
        													.divide((staSatReference.getX().pow(2).add(staSatReference.getY().pow(2))).sqrt());

        // Prepare the estimation
        final EstimatedMeasurement<AngularRaDec> estimated =
                        new EstimatedMeasurement<>(ratesConstructor, iteration, evaluation,
                                                   new SpacecraftState[] {
                                                       transitState
